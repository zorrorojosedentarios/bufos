...
local function ExecuteAdvancedRaidCheck(isSilent)
    ...
    if isSilent then
        ...
        checkKings      = (Assignments.PALADIN_REYES and Assignments.PALADIN_REYES ~= "") or (Assignments.TAMBORES and Assignments.TAMBORES ~= "")
        ...
        checkPriest     = (Assignments.PRIEST and Assignments.PRIEST ~= "") or (Assignments.PERGAMINOS and Assignments.PERGAMINOS ~= "")
    ...
    for _, member in ipairs(unitsToScan) do
        ...
        if member.online and not member.isDead then
            if fileName == "PALADIN" then activePaladins = activePaladins + 1
            elseif fileName == "PRIEST" then activePriests = activePriests + 1
            elseif fileName == "DRUID" then activeDruids = activeDruids + 1
            elseif fileName == "MAGE" then activeMages = activeMages + 1
            end
    ...
    if #activeMageNames >= 2 then
        ...
        if #activeMageNames == 2 then
            table.insert(temp_mageReports, string.format("[MAGOS] %s y %s deben intercambiar Enfoque de Magia.", activeMageNames[1], activeMageNames[2]))
        else
    ...
    if checkWisdom then
        ...
        if sabiCount > 0 then
            local header = (Assignments.PALADIN_SABIDURIA and Assignments.PALADIN_SABIDURIA ~= "") and Assignments.PALADIN_SABIDURIA or "Sabiduría"
    ...
    if checkMight then
        ...
        if poderCount > 0 then
            local header = (Assignments.PALADIN_PODER and Assignments.PALADIN_PODER ~= "") and Assignments.PALADIN_PODER or "Poderío"
    ...
    if checkSalvation then
        ...
        if salvCount > 0 then
            local header = (Assignments.PALADIN_SALVAGUARDA and Assignments.PALADIN_SALVAGUARDA ~= "") and Assignments.PALADIN_SALVAGUARDA or "Salvaguarda"
    ...
    if checkWarrior then
        if #temp_missingGrito > 0 then
            local header = (Assignments.WARRIOR_GRITO and Assignments.WARRIOR_GRITO ~= "") and Assignments.WARRIOR_GRITO or "Grito"
    ...
    if checkDruid then
        if #temp_missingMarca > 0 then
            local header = (Assignments.DRUID_MARCA and Assignments.DRUID_MARCA ~= "") and Assignments.DRUID_MARCA or "DruidaMarca"
    ...
    if checkMage then
        if #temp_missingIntelecto > 0 then
            local header = (Assignments.MAGE_INTELECTO and Assignments.MAGE_INTELECTO ~= "") and Assignments.MAGE_INTELECTO or "MagoIntelecto"
    ...
    if alertCount == 0 then
        ...
        if activePaladins == 0 or activePriests == 0 or activeDruids == 0 or activeMages == 0 then
            ...
            broadcastMsg = "¡Todos los beneficios de las clases presentes están al día! (Sin: " .. table.concat(missingClasses, ", ") .. ")"
        end
...
local function CreateRBCConfigMenu()
        ...
        if field.key == "MAGE_IGNORE_LIST" then
                                ...
                                func = function()
                                    ...
                                    if currentText == "" then
                                        ...
                                        for word in string.gmatch(tempStr, "%S+") do
                                            local existingName = string.upper(string.sub(word, 1, 1)) .. string.lower(string.sub(word, 2))
            ...
            dropBtn:SetScript("OnClick", function(self)
                    ...
                    elseif key:find("PALADIN") and fileName == "PALADIN" then
                        ...
                        for otherKey, otherEditBox in pairs(editBoxes) do
                            if otherKey:find("PALADIN") and otherKey ~= key then
                                ...
                                if otherText ~= "" then
                                    local otherClean = string.upper(string.sub(otherText, 1, 1)) .. string.lower(string.sub(otherText, 2))
                        ...
                        isMatch = true
                    elseif key:find("PRIEST") and fileName == "PRIEST" then isMatch = true
                    elseif key:find("DRUID") and fileName == "DRUID" then isMatch = true
                    elseif key:find("MAGE") and fileName == "MAGE" then isMatch = true
                    elseif key:find("WARRIOR") and fileName == "WARRIOR" then isMatch = true
                    end
                ...
                if not hasPlayers then
                    ...
                    if key:find("PALADIN") then className = "Paladines"
                    elseif key:find("PRIEST") then className = "Sacerdotes"
                    elseif key:find("DRUID") then className = "Druidas"
                    elseif key:find("MAGE") then className = "Magos"
                    elseif key:find("WARRIOR") then className = "Guerreros"
                    else className = "jugadores conectados"
                    end
                    table.insert(menuList, { text = "No hay " .. className .. " en banda", disabled = true, notCheckable = true })
                end
    ...
    saveBtn:SetScript("OnClick", function()
        ...
        if DEFAULT_CHAT_FRAME then
            ...
            DEFAULT_CHAT_FRAME:AddMessage("|cffffd700[RBC Nota] Para asegurar el guardado físico en disco ante una desconexión o caída del servidor, escribe /reload o sal del juego normalmente.|r")
        end
        configFrame:Hide()
    end)
...
-- Crear el botón físico clásico en la UI principal
local rbcButton = CreateFrame("Button", "AdvancedBuffCheckerButton", UIParent, "UIPanelButtonTemplate")
rbcButton:SetSize(140, 35) -- Dimensiones estándar: 140x35 píxeles
...
if RBC_ButtonPosition and RBC_ButtonPosition.relX and RBC_ButtonPosition.relY then
    local parentW = rbcButton:GetParent():GetWidth()
    local parentH = rbcButton:GetParent():GetHeight()
    rbcButton:SetPoint("CENTER", UIParent, "BOTTOMLEFT", RBC_ButtonPosition.relX * parentW, RBC_ButtonPosition.relY * parentH)
else
    rbcButton:SetPoint("CENTER", UIParent, "CENTER", 0, 150)
end
rbcButton:SetText("Escanear Banda")
...
-- ==========================================
local minimapButton = CreateFrame("Button", "RBCMinimapButton", Minimap)
